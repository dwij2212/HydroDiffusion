...
def load_scaler(json_path: str):
    with open(json_path, 'r') as f:
        scaler = json.load(f)
    return scaler


def normalize_multi_features(feature: np.ndarray, variable: str, scaler: dict) -> np.ndarray:
    ...
    if variable == 'inputs':
        ...
        for i, col_name in enumerate(COLUMNS):
            ...
            mean = scaler.get(mean_key, 0.0)
            std = scaler.get(std_key, 1.0)
            normalized[:, i] = (feature[:, i] - mean) / std
    elif variable == 'output':
        mean = scaler.get("QObs(mm/d)_mean", 0.0)
        std = scaler.get("QObs(mm/d)_std", 1.0)
...
def load_forcing_multi(camels_root: PosixPath, basin: str) -> Tuple[pd.DataFrame, int]:
    ...
    # Mapping from raw column names ? normalized names
    rename_map = {
        'prcp(mm/day)': 'prcp',
        'PRCP(mm/day)': 'prcp',
