...
class EncoderDecoderDiffusionWrapper(nn.Module):
    ...
    def __init__(self,
        ...
        '''
        # precipitation embedding
        self.pcp_proj_h = nn.Sequential(
            nn.Linear(1, hidden_size * 2),
            nn.SiLU(),
            nn.Linear(hidden_size * 2, hidden_size)
            )
        self.pcp_proj_c = nn.Sequential(
            nn.Linear(1, hidden_size * 2),
            nn.SiLU(),
            nn.Linear(hidden_size * 2, hidden_size)
            )
    ...
    def forward(self,
        ...
        Returns:
          - v_pred:      (B, H, 1)  the predicted velocity for each step in the horizon
        """
        B, H, _ = x_t.shape
        ...
        if isinstance(encoded_state, tuple) and len(encoded_state) == 2:
            ...
            if (t_h is not None) and (t_c is not None):
                ...
                '''
                h_0 = h_enc + t_h.unsqueeze(0) + emb_precip_h.unsqueeze(0) # unsqueeze t_h gives (1, B, hidden_size) todo
        ...
        if self.decoder_name == 'unet':
            v_pred = self.decoder(x_t, t_h, h_enc, future_pcp, static_attributes)
            
        elif self.decoder_name == 'lstm':
            #dec_in  = torch.cat([x_t, future_pcp, t_seq], dim=-1) #todo, t_emb every time step not a global bias now
            ...
            v_pred = self.output_proj(dec_out)  # (B, H, 1)
            
        elif self.decoder_name == 'ssm':
            # x_t, pcp, and raw fourier features are length-H token groups
            v_pred = self.decoder(               # (B, H, 1)  velocity
                x_past = x_t,             # noisy targets
    ...
    def sample_ddim(self,
        ...
        for i in range(num_steps - 1, -1, -1):
            ...
            if self.decoder_name == 'unet':
                v_pred = self.decoder(x, t_h, h_enc, future_pcp, static_attributes) # todo, static_attributes or None. If None, not using static attributes in unet
            elif self.decoder_name == 'lstm':
                #dec_in  = torch.cat([x, future_pcp, t_seq], dim=-1) #todo, t_emb every time step not a global bias now
                
                dec_in  = torch.cat([x, future_pcp], dim=-1) #todo, test concatenation with t_emb still global bias
            ...
            x0  = alpha_t * x - sigma_t * v_pred # x0: (B, H, 1)
            eps = (v_pred + sigma_t * x0) / alpha_t
