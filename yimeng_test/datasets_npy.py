...
class CamelsNPY(Dataset):
    ...
    z_emb_path : str                        # ← NEW
        Path to the CAE embedding .npy file (531, 32).
        If empty string (default), embedding is not used and __getitem__
        returns the same tuple as before — no other code is affected.
    """
    ...
    def __init__(
        ...
        is_train: bool = True,
        z_emb_path: str = "",               # ← NEW: path to CAE embeddings
        model_name: str = "lstm",
        **kwargs,            # absorb unused keys so callers don't need to filter
        ...
        # ---- NEW: load CAE embeddings if path provided ----
        self.z_emb = None
        if z_emb_path:
            z_raw = np.load(z_emb_path)
            self.z_emb = z_raw.astype(np.float32)  # (531, 32)
    ...
    def __getitem__(self, idx: int):
        ...
        if self.no_static:
            ...
            # ---- NEW: return z_emb alongside attrs if embeddings are loaded ----
            if self.z_emb is not None:
                z_emb = torch.from_numpy(self.z_emb[b_row])       # (32,)
                return x_t, attrs, z_emb, y_t, q_m, q_s, basin, date
            else:
